...
export function ChatCard({
  ...
  const { messages, fetchMessages, sendMessage, loading, error } =
    useMessageStore();
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {loading ? (
          ...
          filteredMessages.map((message, index) => (
            <div
              key={`message-${message.id}-${message.sent_at}-${index}`}
...
export function OnlineMembersPopover({ communityId }: { communityId: number }) {
  ...
  const { onlineCount, totalMembers, onlineMembers } =
      ...
      <PopoverContent className="w-96 p-0">
        <div className="p-4 border-b">
          <div className="flex items-center space-x-2">
            <h4 className="text-sm font-semibold flex-1">Community Members</h4>
